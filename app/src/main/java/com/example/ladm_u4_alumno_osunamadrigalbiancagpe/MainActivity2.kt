...
import com.example.ladm_u4_alumno_osunamadrigalbiancagpe.databinding.ActivityMain2Binding
import com.google.android.material.snackbar.Snackbar
import java.time.LocalDateTime
import java.time.format.DateTimeFormatter

class MainActivity2 : AppCompatActivity(), DevicesRecyclerViewAdapter.ItemClickListener,
    ...
    fun asistenciaHoy(){
        val nc = this.intent.getStringExtra("NC").toString()
        val name = this.intent.getStringExtra("NAME").toString()

        var asist = Registros(this)
        asist = asist.buscarAsistencia(fechaActual())

        if(asist.fecha!=""){
            AlertDialog.Builder(this).setMessage("YA REGISTRÓ ASISTENCIA EL DIA DE HOY").show()
        }else{
            val hr = horaActual().split(":")
            asist.fecha = fechaActual()
            asist.hora = hr[0]
            asist.minutos = hr[1]

            if(asist.insertarAsistencia()){
                val cad = "${nc}¬${name}¬${fechaActual()}¬${hr[0]}¬${hr[1]}"
                sendMessage(cad)
            }
        }
    }
        ...
        override fun handleMessage(msg: android.os.Message) {
            ...
            when (msg.what) {
                ...
                Constants.MESSAGE_STATE_CHANGE -> {
                    ...
                    when (msg.arg1) {
                        BluetoothChatService.STATE_CONNECTED -> {
                            ...
                            binding.imgConnectionDot.setImageDrawable(getDrawable(R.drawable.ic_circle_connected))
                            Snackbar.make(findViewById(R.id.lyMain),"Conectado a ${mConnectedDeviceName}",
                                Snackbar.LENGTH_SHORT).show()
                            binding.btnPresente.visibility = View.VISIBLE
                            binding.lblPresente.visibility = View.VISIBLE
                            connected = true
                        }
                        BluetoothChatService.STATE_CONNECTING -> {
                            ...
                            binding.btnPresente.visibility = View.GONE
                            binding.lblPresente.visibility = View.GONE
                            connected = false
                        }
                        BluetoothChatService.STATE_LISTEN, BluetoothChatService.STATE_NONE -> {
                            binding.lblStatus.text = getString(R.string.not_connected)
                            binding.imgConnectionDot.setImageDrawable(getDrawable(R.drawable.ic_circle_red))
                            Snackbar.make(findViewById(R.id.lyMain),getString(R.string.not_connected),
                                Snackbar.LENGTH_SHORT).show()
                            binding.btnPresente.visibility = View.GONE
                            binding.lblPresente.visibility = View.GONE
                            connected = false
                ...
                Constants.MESSAGE_DEVICE_NAME -> {
                    ...
                    binding.lblStatus.text = getString(R.string.connected_to) + " " +mConnectedDeviceName
                    binding.imgConnectionDot.setImageDrawable(getDrawable(R.drawable.ic_circle_connected))
                    Snackbar.make(findViewById(R.id.lyMain),"Connected to " + mConnectedDeviceName,
                        Snackbar.LENGTH_SHORT).show()
                    binding.btnPresente.visibility = View.VISIBLE
                    binding.lblPresente.visibility = View.VISIBLE
                    connected = true
